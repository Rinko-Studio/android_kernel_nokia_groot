...
int ili_fw_dump_iram_data(u32 start, u32 end, bool save)
{
	...
	fw_buf = kzalloc(MAX_HEX_FILE_SIZE, GFP_DMA);
...
static int ilitek_fw_check_ddi_chunk(u8 *pfw)
{
	...
	ILI_DBG("Read customer reserved block: start addr = 0x%X, len = 0x%X ", start, DDI_RSV_BK_SIZE);
	...
	cmd[5] = (start >> 16) & 0xFF; 	//addr H byte
	cmd[6] = ili_calc_packet_checksum(cmd, sizeof(cmd) - 1);
	ILI_DBG("Read mp data block: start addr = 0x%X, len = 0x%X ", start, DDI_RSV_BK_SIZE);
	if (ilits->wrapper(cmd, sizeof(cmd), fw_flash_tmp, sizeof(fw_flash_tmp), OFF, ON) < 0) {
...
static int ilitek_tddi_fw_check_ver(u8 *pfw)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		ILI_DBG("Block = %d, HW CRC = 0x%06x, Flash CRC = 0x%06x\n", i, hw_crc[i], flash_crc_cb);
	...
	/* Check FW version */
	ILI_DBG("New FW ver = 0x%x, Current FW ver = 0x%x\n", tfd.new_fw_cb, ilits->chip->fw_ver);
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		len = fbi[i].end - fbi[i].start + 1 - 4;
		hex_crc = CalculateCRC32(fbi[i].start, len, pfw);
		ILI_DBG("Block = %d, HW CRC = 0x%06x, Hex CRC = 0x%06x\n", i, hw_crc[i], hex_crc);
		if (hex_crc != hw_crc[i]) {
	...
	ILI_DBG("Firmware is the newest version, upgrade abort\n");
	return UPDATE_PASS;
}
...
static int ilitek_tddi_fw_flash_program(u8 *pfw)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		ILI_DBG("Block[%d]: Programing from (0x%x) to (0x%x), tfd.end_addr = 0x%x\n",
...
static int ilitek_tddi_fw_flash_erase(void)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		ILI_DBG("Block[%d]: Erasing from (0x%x) to (0x%x) \n", i, fbi[i].start, fbi[i].end);
...
static void ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	ILI_DBG("Parsing hex info start addr = 0x%x\n", fw_info_addr);
	...
	ILI_DBG("Parsing hex mp ver addr = 0x%x\n", fw_mp_ver_addr);
	...
	ILI_DBG("New FW ver = 0x%x\n", tfd.new_fw_cb);
	ILI_DBG("star_addr = 0x%06X, end_addr = 0x%06X, Block Num = %d\n", tfd.start_addr, tfd.end_addr, tfd.block_number);
}
...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
	...
	ILI_DBG("Open file method = %s, path = %s\n",
	...
	case REQUEST_FIRMWARE:
		if (request_firmware(&fw, ilits->md_fw_rq_path, ilits->dev) < 0) {
			ILI_DBG("Request firmware failed, try again\n");
			if (request_firmware(&fw, ilits->md_fw_rq_path, ilits->dev) < 0) {
		...
		break;
	case FILP_OPEN:
		if (!ilits->md_fw_filp_path) {
			ILI_ERR("the md_fw_filp_path is NULL pointer\n");
			ret = -1;
			goto out;
		}
...
int ili_fw_upgrade(int op)
{
	...
	if (!ilits->boot || ilits->force_fw_update || ERR_ALLOC_MEM(pfw)) {
		...
		if (ilitek_tdd_fw_hex_open(op, pfw) < 0) {
			ILI_DBG("Open hex file fail, try upgrade from ILI file\n");
