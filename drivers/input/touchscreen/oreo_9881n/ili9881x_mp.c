...
/* char M_CFG_NIS = ':'; */
	...
	ini_buf = kzalloc((PARSER_MAX_CFG_BUF + 1) * sizeof(char), GFP_DMA);
	...
	tmpSectionName = kzalloc((PARSER_MAX_CFG_BUF + 1) * sizeof(char), GFP_DMA);
...
static int allnode_key_cdc_data(int index)
{
	...
	ori = kcalloc(len, sizeof(u8), GFP_DMA);
	...
	if (key_buf == NULL) {
		key_buf = kcalloc(core_mp.key_len, sizeof(s32), GFP_DMA);
...
static int allnode_mutual_cdc_data(int index)
{
	...
	if (frame_buf == NULL) {
		frame_buf = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
...
static int create_mp_test_frame_buffer(int index, int frame_count)
{
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		if (core_mp.tx_delta_buf == NULL) {
			core_mp.tx_delta_buf = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		...
		if (core_mp.rx_delta_buf == NULL) {
			core_mp.rx_delta_buf = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		...
		if (core_mp.tx_max_buf == NULL) {
			core_mp.tx_max_buf = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		...
		if (core_mp.tx_min_buf == NULL) {
			core_mp.tx_min_buf = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		...
		if (core_mp.rx_max_buf == NULL) {
			core_mp.rx_max_buf = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		...
		if (core_mp.rx_min_buf == NULL) {
			core_mp.rx_min_buf = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		...
		if (tItems[index].result_buf == NULL) {
			tItems[index].result_buf = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		...
		if (tItems[index].max_buf == NULL) {
			tItems[index].max_buf = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		...
		if (tItems[index].min_buf == NULL) {
			tItems[index].min_buf = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		...
		if (tItems[index].spec_option == BENCHMARK) {
			if (tItems[index].bench_mark_max == NULL) {
				tItems[index].bench_mark_max = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
			...
			if (tItems[index].bench_mark_min == NULL) {
				tItems[index].bench_mark_min = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
...
static int open_test_sp(int index)
{
	...
	char str[512] = {0};
	struct mp_test_P540_open *open = kzalloc(sizeof(struct mp_test_P540_open)*tItems[index].frame_count, GFP_DMA);
	if ( open == NULL ) {
		ILI_ERR("alloc mem for open failed\n");
		return -EMP_NOMEM;
	}
	ILI_DBG("index = %d, desp = %s, Frame Count = %d\n",
		index, tItems[index].desp, tItems[index].frame_count);
	...
	if (frame1_cbk700 == NULL) {
		frame1_cbk700 = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
	...
	if (frame1_cbk250 == NULL) {
		frame1_cbk250 = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
	...
	if (frame1_cbk200 == NULL) {
		frame1_cbk200 = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
	...
	tItems[index].node_type = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		open[i].tdf_700 = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		open[i].tdf_250 = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		open[i].tdf_200 = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		open[i].cbk_700 = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		open[i].cbk_250 = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		open[i].cbk_200 = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		open[i].charg_rate = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		open[i].full_Open = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		open[i].dac = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
	}
...
static int open_test_cap(int index)
{
	...
	int i = 0, x = 0, y = 0, ret = 0, addr = 0;
	char str[512] = {0};
	struct mp_test_open_c *open = kzalloc(sizeof(struct mp_test_open_c)*tItems[index].frame_count, GFP_DMA);
	if ( open == NULL ) {
		ILI_ERR("alloc mem for open failed\n");
		return -EMP_NOMEM;
	}
	...
	if (cap_dac == NULL) {
		cap_dac = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
	...
	if (cap_raw == NULL) {
		cap_raw = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		open[i].cap_dac = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		open[i].cap_raw = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
		open[i].dcl_cap = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
	}
...
static int mp_test_data_sort_average(s32 *oringin_data, int index, s32 *avg_result)
{
	...
	u32data_buff = kcalloc(core_mp.frame_len * tItems[index].frame_count, sizeof(s32), GFP_DMA);
	u32sum_raw_data = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
...
static int mp_compare_test_result(int index)
{
	...
	max_threshold = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
	...
	min_threshold = kcalloc(core_mp.frame_len, sizeof(s32), GFP_DMA);
...
static void ilitek_tddi_mp_init_item(void)
{
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		...
		tItems[i].result = kmalloc(16, GFP_DMA);
