...
static int iris_dbc_lut_read(const u8 *fw_data)
{
	...
	if (dbc_lut_income_send_cmd == NULL) {
		dbc_lut_income_send_cmd = vzalloc(sizeof(struct dsi_cmd_desc)*AB_TABLE*dbc_income_pkt_cnt);
	...
	if (dbc_lut_compk_send_cmd == NULL) {
		dbc_lut_compk_send_cmd = vzalloc(sizeof(struct dsi_cmd_desc)*AB_TABLE*dbc_compk_pkt_cnt*DBC_LEVEL_CNT);
	...
	if (dbc_lut_cabc_dlv_send_cmd == NULL) {
		dbc_lut_cabc_dlv_send_cmd = vzalloc(sizeof(struct dsi_cmd_desc)*dbc_dlv_pkt_cnt*DBC_LEVEL_CNT);
	...
	if (iris_dbc_lut_buf == NULL)
		iris_dbc_lut_buf = vzalloc(dbc_pkt_size*ucmds_cnt);
...
int iris_cm_lut_read(u32 lut_table_index, const u8 *fw_data)
{
	...
	if (iris_cm_lut_buf == NULL)
		iris_cm_lut_buf = vzalloc(cm_pkt_size*cm_one_lut_pkt_cnt*CM_LUT_NUMBER);

	if(cm_lut_send_cmd == NULL) {
		cm_lut_send_cmd = vzalloc(sizeof(struct dsi_cmd_desc)*cm_one_lut_pkt_cnt*CM_LUT_NUMBER);
...
static int iris_sdr2hdr_lut_read(u32 lut_group_index, const u8 *fw_data)
{
	...
	if (sdr2hdr_lut_send_cmd == NULL) {
		sdr2hdr_lut_send_cmd = vzalloc(sizeof(struct dsi_cmd_desc)*hdr_one_lut_pkt_cnt*SDR2HDR_LUT_GROUP_CNT);
	...
	if (sdr2hdr_inv_uv_send_cmd == NULL) {
		sdr2hdr_inv_uv_send_cmd = vzalloc(sizeof(struct dsi_cmd_desc)*uv_one_lut_pkt_cnt*SDR2HDR_LUT_GROUP_CNT);
	...
	if (iris_sdr2hdr_lut_buf == NULL) {
		iris_sdr2hdr_lut_buf = vzalloc(one_group_size*SDR2HDR_LUT_GROUP_CNT);
	}
...
static int iris_scaler1d_lut_read(u32 lut_table_index, const u8 *fw_data)
{
	...
	if (scaler1d_lut_send_cmd == NULL) {
		scaler1d_lut_send_cmd = vzalloc(sizeof(struct dsi_cmd_desc)*scaler_one_lut_pkt_cnt*SCALER1D_LUT_NUMBER);
	...
	if (iris_scaler1d_lut_buf == NULL)
		iris_scaler1d_lut_buf = vzalloc(scaler_pkt_size*scaler_one_lut_pkt_cnt*SCALER1D_LUT_NUMBER);
...
static int iris_gamma_lut_read(u32 lut_table_index, const u8 *fw_data)
{
	...
	if (gamma_lut_send_cmd == NULL) {
		gamma_lut_send_cmd = vzalloc(sizeof(struct dsi_cmd_desc)*gamma_one_lut_pkt_cnt*GAMMA_LUT_NUMBER);
	...
	if (iris_gamma_lut_buf == NULL)
		iris_gamma_lut_buf = vzalloc(gamma_pkt_size*gamma_one_lut_pkt_cnt*GAMMA_LUT_NUMBER);
...
void iris_ambient_lut_update(enum LUT_TYPE lutType)
{
	...
	if (dynamic_lut_send_cmd == NULL) {
		dynamic_lut_send_cmd = vzalloc(sizeof(struct dsi_cmd_desc)*hdr_block_pkt_cnt*SDR2HDR_LUT2_BLOCK_NUMBER);
	...
	if (!iris_ambient_lut_buf)
		iris_ambient_lut_buf = vzalloc(hdr_pkt_size*iris_lut_param.hdr_lut2_pkt_cnt);
...
void iris_maxcll_lut_update(enum LUT_TYPE lutType)
{
	...
	if (dynamic_lutuvy_send_cmd == NULL) {
		dynamic_lutuvy_send_cmd = vzalloc(sizeof(struct dsi_cmd_desc)*hdr_block_pkt_cnt*SDR2HDR_LUTUVY_BLOCK_NUMBER);
...
int iris_gamma_lut_update(u32 lut_table_index, const u32 *fw_data)
{
	...
	gammaformat = vmalloc(sizeof(u16) * GAMMA_LUT_LENGTH);
