...
static int32_t iris_pt_alloc_cmds_space(struct dsi_panel_cmd_set *cmdset,
		struct dsi_cmd_desc **ptx_cmds, struct iris_ocp_cmd **pocp_cmds)
{
	...
	*ptx_cmds = vmalloc(cmds_cnt *sizeof(**ptx_cmds));
	if (!(*ptx_cmds)) {
		IRIS_LOGE("can not vmalloc len = %lu\n", cmds_cnt * sizeof(**ptx_cmds));
		return -ENOMEM;
	}
...
void iris_panel_cmd_passthrough_wr(struct dsi_panel *panel, struct dsi_panel_cmd_set *cmdset)
{
	...
	if (cmds_cnt < 0) {
		pr_err("cmdset cmds allocate failed!\n");
		return;
	}
