...
static int mdss_mipi_dsi_command(void __user *values)
{
	...
	if (cmd.dtype == 0x0f) {
		...
		pdesc_muti = vmalloc(sizeof(struct dsi_cmd_desc) * cmd_cnt);
...
int iris_configure(u32 type, u32 value)
{
	...
	if (type != IRIS_ANALOG_BYPASS_MODE && pcfg->valid < IRIS_STATUS_FULL_LIGHTUP) {
	...
	case IRIS_HDR_PREPARE:
		if (pcfg->add_last_flag_speed_up == 0 && pcfg->add_last_flag == pcfg->add_on_last_flag) {
			pcfg->add_last_flag_speed_up = 1;
			pcfg->add_last_flag = pcfg->add_pt_cont_last_flag;
			IRIS_LOGI("%s: IRIS_HDR_PREPARE speed up\n", __func__);
		}
	...
	case IRIS_FW_UPDATE:
		...
		if (value == 0) {
			iris3_parse_lut_cmds(&pcfg->display->pdev->dev, IRIS3_FIRMWARE_DEFAULT_NAME);
			if (pcfg->lut_mode == INTERPOLATION_MODE) {
				iris_cm_color_gamut_set(pqlt_cur_setting->pq_setting.cmcolorgamut);
			} else {
				pcfg->valid = IRIS_STATUS_MINI_LIGHTUP; /* minimum light up */
				iris3_send_cont_splash_pkt(IRIS_CONT_SPLASH_KERNEL_CAL);
				pcfg->valid = IRIS_STATUS_FULL_LIGHTUP; /* full light up */
			}
		} else if (value == 1) {
			iris3_parse_lut_cmds(&pcfg->display->pdev->dev, IRIS3_FIRMWARE_DEFAULT_NAME_2ND);
			if (pcfg->lut_mode == INTERPOLATION_MODE) {
				iris_cm_color_gamut_set(pqlt_cur_setting->pq_setting.cmcolorgamut);
			} else {
				pcfg->valid = IRIS_STATUS_MINI_LIGHTUP; /* minimum light up */
...
int iris3_operate_conf(struct msm_iris_operate_value *argp)
{
	...
	if (pcfg->valid < IRIS_STATUS_MINI_LIGHTUP) {
		if (child_type == IRIS_DBG_KERNEL_LOG_LEVEL
			|| child_type == IRIS_WORK_MODE || child_type == IRIS_CHIP_VERSION || child_type == IRIS_ANALOG_BYPASS_MODE)
			;
		else
			return -EINVAL;
	}
