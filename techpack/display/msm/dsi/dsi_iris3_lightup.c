                                 ...
                                 const struct iris_cmd_statics *pcmd_statics)
{
	...
	pcmds->cmds = vzalloc(pcmd_statics->cnt * sizeof(struct dsi_cmd_desc));
	if (!pcmds->cmds) {
		IRIS_LOGE("can not vzalloc space for dsi\n");
		return -ENOMEM;
	}
	...
	for (i = 0; i < IRIS_IP_CNT; i++) {
		...
		if ( opt_cnt != 0) {
			pip_index[i].opt = kvzalloc(opt_cnt * sizeof(struct iris_ip_opt), GFP_KERNEL);
			if (! pip_index[i].opt) {
					 ...
					 struct dsi_cmd_desc * pdesc, int mult)
{
	...
	if (mult == 1) {
		...
		ptr = (uint8_t *) vzalloc(dlen);
		if (!ptr) {
			IRIS_LOGE("can not vzalloc space \n");
			return -ENOMEM;
		}
...
static int32_t iris_parse_color_temp_info(struct device_node *np, struct iris_cfg * pcfg)
{
	...
	IRIS_LOGD("pxlw,P3_color_temp: %d\n", pcfg->P3_color_temp);
	...
	IRIS_LOGD("pxlw,sRGB-color-temp: %d\n", pcfg->sRGB_color_temp);
	...
	IRIS_LOGD("pxlw,sdr2hdr-color-temp: %d\n", pcfg->sdr2hdr_color_temp);
	...
	IRIS_LOGD("pxlw,hdr-color-temp: %d\n", pcfg->hdr_color_temp);
...
static int32_t iris_parse_cmd_list(struct device_node *np, struct iris_cfg * pcfg) {
	...
	buf = (uint32_t *)vzalloc(sizeof(char) * blen);
	if (!buf) {
...
static int32_t iris_alloc_cmd_seq_common(struct iris_ctrl_seq  * pctrl_seq, int32_t seq_cnt)
{
	...
	if (pctrl_seq->ctrl_opt == NULL) {
		IRIS_LOGE("can not vmalloc space for pctrl opt\n");
		return -ENOMEM;
	}
...
static int32_t iris_alloc_pq_init_space(struct iris_cfg *pcfg, const uint8_t *pdata, int32_t blen)
{
	...
	ptr = (int32_t *)vmalloc(sizeof(*ptr) * (blen >>1));
	if (ptr == NULL) {
		IRIS_LOGE("can not vmalloc space for ptr\n");
		return -EINVAL;
	}
	...
	pinit_val->val = (struct iris_pq_ipopt_val *) vmalloc(sizeof(struct iris_pq_ipopt_val) * ip_cnt);
	if (pinit_val->val == NULL) {
		IRIS_LOGE("can not vmalloc pinit_val->val\n");
		rc = -EINVAL;
		goto EXIT_FREE;
	}
	...
	for (i = 0; i < ip_cnt; i++) {
		...
		pinit_val->val[i].popt = vmalloc(sizeof(uint8_t) * ptr[i]);
	}
...
static void __cont_splash_work_handler(struct work_struct *work)
{
	...
	iris3_send_cont_splash_pkt(IRIS_CONT_SPLASH_KERNEL_CAL);
...
int iris3_parse_params(struct device_node *np, struct dsi_panel *panel)
{
	...
	if (rc) {
		...
		pcfg->max_color_temp= 10350;
		pcfg->P3_color_temp= 7800;
		pcfg->sRGB_color_temp= 7800;
		pcfg->sdr2hdr_color_temp= 7800;
		pcfg->hdr_color_temp= 7800;
	}
...
void  iris_print_cmds(struct dsi_cmd_desc *p, int num, int state)
{
	...
	for (i = 0; i < num; i++) {
		...
		arr = (uint8_t *)vmalloc(len * sizeof(uint8_t));
		if (! arr) {
			IRIS_LOGE("the vmalloc is error\n");
			return;
		}
...
void iris_alloc_seq_space(void)
{
	...
	pdesc =
		(struct dsi_cmd_desc *)vmalloc(sum  * sizeof(struct dsi_cmd_desc));
	if (!pdesc) {
		IRIS_LOGE("can not vmalloc buffer \n");
		return;
	}
...
u8 panel_calibrate_status = READ_MCF_INIT;
#if 0
static int iris_read_mcf_from_panel(struct dsi_panel *panel, u32 size, u8 *pvalues)
{
	...
	IRIS_LOGD("%s\n", __func__);
	len = iris3_1st_panel_mcf_read(panel, size, pvalues);
...
*/
#define STATIC_VALUE_TEST 0
int iris3_1st_panel_mcf_read(struct dsi_panel *panel, u32 size, char *buf)
{
	int /*rc = 0,*/ len = 0;
	u8 read_buf[128] = {0};
#if STATIC_VALUE_TEST
	u8 static_value[]={204, 12, 128, 122, 7, 191, 5, 192, 175, 96, 78, 234, 65, 91, 175, 208, 38, 80, 9, 190, 76, 27, 80, 21, 111, 247, 115, 160, 115, 227, 126, 125, 127, 58, 127, 57, 127, 198, 126, 71, 127, 192, 127, 189, 127, 132, 127, 100, 51, 195, 51, 238, 51, 135, 55, 209, 54, 30, 55, 2, 55, 145, 55, 226, 54, 32, 106, 119, 61, 245, 64, 165, 47, 255, 255, 96, 16, 144, 1, 9, 160, 156, 56, 4, 13, 192, 129, 32, 195, 4, 224, 72, 72, 193, 251, 0, 0, 0, 0, 0, 0, 0, 0, 0, 251, 127 };
#else
	u8 data;
#endif
	int i;
#if !STATIC_VALUE_TEST
	struct iris_cfg *pcfg = iris_get_cfg();
	char cmd1[4] = {0xff, 0x78, 0x07, 0x0b};
	struct dsi_cmd_desc otp_cmds[4] = {
	...
	char write_read_bank_cmd[2] = {0xFD, 0x00};
	struct dsi_cmd_desc write_read_cmds_set = {
		{0, 0x06, 0, 0, 0, sizeof(write_read_bank_cmd), write_read_bank_cmd, 2, &data}, 1, 1};
	struct dsi_panel_cmd_set write_read_cmdset = {
		.state = DSI_CMD_SET_STATE_LP,
		.count = 1,
		.cmds = &write_read_cmds_set,
	};
	char write_read_mcf_cmd[2] = {0x00, 0x00};
	struct dsi_cmd_desc write_read_mcf_cmds_set = {
		{0, 0x06, 0, 0, 0, sizeof(write_read_mcf_cmd), write_read_mcf_cmd, 2, &data}, 1, 1};
	struct dsi_panel_cmd_set write_read_mcf_cmdset = {
		.state = DSI_CMD_SET_STATE_LP,
		.count = 1,
		.cmds = &write_read_mcf_cmds_set,
	};
	
	char cmd2[4] = {0xff, 0x78, 0x07, 0x00};
	struct dsi_cmd_desc otp_cmds_2[4] = {
		{{0, 0x39, 0, 0, 0, sizeof(cmd2), cmd2, 0, NULL}, 0, 1},
	};

	struct dsi_panel_cmd_set otp_cmdset_2 = {
		.state = DSI_CMD_SET_STATE_LP,
		.count = 1,
		.cmds = otp_cmds_2,
	};
...
#endif
	IRIS_LOGI("Start read otp cmd\n");
#if !STATIC_VALUE_TEST
	if (pcfg->abypss_ctrl.abypass_mode == PASS_THROUGH_MODE) {
		iris3_panel_cmd_passthrough(panel, &otp_cmdset);
	} else {
		iris3_dsi_cmds_send(panel, otp_cmdset.cmds, otp_cmdset.count, otp_cmdset.state);
	}
	if (pcfg->abypss_ctrl.abypass_mode == PASS_THROUGH_MODE) {
		iris3_panel_cmd_passthrough(panel, &write_read_cmdset);
	} else {
		iris3_dsi_cmds_send(panel, write_read_cmdset.cmds, write_read_cmdset.count, write_read_cmdset.state);
	}
	IRIS_LOGI("Page switch to page = %d \n", data);

#endif
	for (i=0;i<size;i++) {
#if !STATIC_VALUE_TEST
		write_read_mcf_cmd[0] = i;
		if (pcfg->abypss_ctrl.abypass_mode == PASS_THROUGH_MODE) {
			iris3_panel_cmd_passthrough(panel, &write_read_mcf_cmdset);
		} else {
			iris3_dsi_cmds_send(panel, write_read_mcf_cmdset.cmds, write_read_mcf_cmdset.count, write_read_mcf_cmdset.state);
		}
		read_buf[i] = data;
#else
		read_buf[i] = static_value[i];
#endif
	IRIS_LOGI("MCF[%d] = %d \n", i, read_buf[i]);
	}
#if !STATIC_VALUE_TEST
	if (pcfg->abypss_ctrl.abypass_mode == PASS_THROUGH_MODE) {
		iris3_panel_cmd_passthrough(panel, &otp_cmdset_2);
	} else {
		iris3_dsi_cmds_send(panel, otp_cmdset_2.cmds, otp_cmdset_2.count, otp_cmdset_2.state);
	}
	...
	IRIS_LOGI("read otp  len = %d\n", len);
	return len;
}
EXPORT_SYMBOL(iris3_1st_panel_mcf_read);
...
extern int dsi_panel_reset_delay(struct dsi_panel *panel);
...
int iris_panel_enable(struct dsi_panel *panel, struct dsi_panel_cmd_set *on_cmds)
{
	...
	if (iris3_abypass_mode_get() == PASS_THROUGH_MODE) {
		...
		IRIS_LOGI("%s iris3\n", __func__);
		iris_rec_time(1);
		rc = dsi_panel_reset_delay(panel);
	...
	if (panel_calibrate_state_get() == READ_MCF_READY) {
		if (1) {//(!strcmp(display->name, "")) {
			iris3_1st_panel_mcf_read(panel, 106, panel_mcf_data);
			panel_mcf_data[127] = 0x11;
		} else {
			panel_mcf_data[127] = 0x22;
		}
		iris_rec_time(1);
		panel_calibrate_state_set(READ_MCF_DONE);
	}
	iris_read_power_mode(panel);
...
void iris3_send_cont_splash_pkt(uint32_t type)
{
	...
	} else if (type == IRIS_CONT_SPLASH_KERNEL_CAL) {
		len = iris_select_cont_splash_ipopt(type, opt_arr);
		pcfg->add_last_flag = pcfg->add_cont_last_flag;
		iris_send_assembled_pkt(opt_arr, len);
		iris_lp_set(); /* set iris low power */
...
void iris3_display_prepare(struct dsi_display *display)
{
	...
	if (iris_boot == false) {
		if (strcmp(pcfg->display->name, "qcom,mdss_dsi_ili7807q_txd_fhd_video") == 0) // 1st panel
			iris3_parse_lut_cmds(&pcfg->display->pdev->dev, IRIS3_FIRMWARE_NAME);
		else
			iris3_parse_lut_cmds(&pcfg->display->pdev->dev, IRIS3_FIRMWARE_NAME_2ND);
...
{
	char *str = NULL;
	//default iris in PT mode
	unsigned long cmdline_iris_mode = 0;

	display->cmdline_iris_mode = cmdline_iris_mode;
	str = strnstr(boot_str, ":iris_mode", strlen(boot_str));
	if (!str) {
		IRIS_LOGD("%s do not set iris_mode\n", __func__);
		return;
	}

	if (kstrtol(str + strlen(":iris_mode"), 10,
			(unsigned long *)&cmdline_iris_mode)) {
		IRIS_LOGD("invalid timing index override: %s. resetting iris mode  timing and config\n", boot_str);
		return;
	}

	IRIS_LOGD("successfully iris_mode =%ld\n", cmdline_iris_mode);
	display->cmdline_iris_mode = cmdline_iris_mode;
}
