...
static int iris_i2c_cmd_four_read(struct addr_val * val, int len)
{
	...
	nine_data_list = vmalloc(9 * len * sizeof(nine_data_list[0]));
	if (!nine_data_list)
		return -ENOMEM;

	msgs = vmalloc(2 * len * sizeof(msgs[0]));
	if (!msgs) {
...
static int iris_i2c_cmd_four_write(struct addr_val * val, int len)
{
	...
	msgs = vmalloc(len * sizeof(msgs[0]));
	if (!msgs) {
		kfree(nine_data_list);
		nine_data_list = NULL;
		return -ENOMEM;
...
static int iris_i2c_single_conver_ocp_read(uint32_t *ptr, uint32_t len)
{
	...
	u8 * p = NULL;
	p = vmalloc(sizeof(*val_tmp) * (len+1));
	if (NULL == p) {
...
static int iris_i2c_burst_conver_ocp_read(uint32_t *ptr, uint32_t dlen)
{
	...
	iris_payload = vmalloc(sizeof(iris_payload[0]) * (5+msg_len));
...
int iris_i2c_single_conver_ocp_write(uint32_t *arr, uint32_t dlen)
{
	...
	p = vmalloc(sizeof(*val_tmp) * (dlen)+1);
	if (NULL == p) {
...
int iris_i2c_burst_conver_ocp_write(uint32_t base_addr, uint32_t *arr, uint32_t dlen)
{
	...
	iris_payload = vmalloc(sizeof(iris_payload[0]) * msg_len);
	if (NULL == iris_payload) {
		IRIS_LOGE("[iris3] %s: allocate memory fails\n", __func__);
		return -EINVAL;
	}
...
int iris_i2c_group_write(struct iris_i2c_msg * iris_i2c_msg, uint32_t iris_i2c_msg_num)
{
	...
	msgs = vmalloc( sizeof(struct i2c_msg) * iris_i2c_msg_num + 1 );
	if ( NULL == msgs ) {
